...
uint8_t gc_execute_line(char *line) {
    ...
    uint8_t word_bit = 0xff; // Bit-value for assigning tracking variables
                        ...
                        if (servo_enable(gc_block.values.p, bit_istrue(value_words, bit(WORD_T)) ? gc_block.values.t : PWM_SERVO_TYPE_ANALOG)) {
                            ...
                            if (angle < 0.0) {
                                ...
                                if (angle < 200.0) {
                                    ...
                                    if (!pwm_setPulseLength(gc_block.values.p, angle)) {
                                        FAIL(STATUS_GCODE_INVALID_TARGET);
                                    }
                                }
                            }
                    ...
                    if (bit_istrue(value_words, bit(WORD_I))) { // If I(nhibit) parameter present, PWM channel will be shut down
                        ...
                        if (pwm_isChannelEnabled(gc_block.values.p)) {
                            ...
                            if (pwm_enable(gc_block.values.p, gc_block.values.r)) {
                                ...
                                FAIL(STATUS_BAD_NUMBER_FORMAT); //Channel cannot be enabled
                            }
                        }
                    }
                    pwm_applyChannel(gc_block.values.p);
